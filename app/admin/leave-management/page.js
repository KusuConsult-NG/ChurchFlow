...
export default function LeaveManagement() {
  ...
  const leaveTypes = [
    { id: 'ANNUAL', name: 'Annual Leave', maxDays: 21, color: 'bg-blue-100 text-blue-800' },
    { id: 'SICK', name: 'Sick Leave', maxDays: 10, color: 'bg-red-100 text-red-800' },
    { id: 'PERSONAL', name: 'Personal Leave', maxDays: 5, color: 'bg-green-100 text-green-800' },
    { id: 'MATERNITY', name: 'Maternity Leave', maxDays: 90, color: 'bg-pink-100 text-pink-800' },
    { id: 'PATERNITY', name: 'Paternity Leave', maxDays: 14, color: 'bg-purple-100 text-purple-800' },
    { id: 'COMPASSIONATE', name: 'Compassionate Leave', maxDays: 7, color: 'bg-gray-100 text-gray-800' },
    { id: 'STUDY', name: 'Study Leave', maxDays: 30, color: 'bg-yellow-100 text-yellow-800' },
    { id: 'EMERGENCY', name: 'Emergency Leave', maxDays: 3, color: 'bg-orange-100 text-orange-800' }
  ];
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Manage staff leave requests and approvals</p>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {leaveRequests.map((request) => (
                <tr key={request.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`status-badge ${
                      statusOptions.find(s => s.id === request.status)?.color || 'bg-gray-100 text-gray-800'
                    }`}>
                      {statusOptions.find(s => s.id === request.status)?.name || request.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    {leaveTypes.map(type => (
                      <option key={type.id} value={type.id}>{type.name} (Max: {type.maxDays} days)</option>
                    ))}
                  </select>
                </div>
              ...
              {formData.startDate && formData.endDate && (
                <div className="bg-blue-50 p-4 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Leave Duration:</strong> {calculateLeaveDays(formData.startDate, formData.endDate)} days
                  </p>
                </div>
              )}
