...
export default async function BankReconciliation() {
  ...
  if (
    !session?.user ||
    !['BANK_OPERATOR', 'ADMIN'].includes(session.user.role)
  ) {
    return (
      <main className='p-6'>
  ...
  const [accounts, pendingTransactions, reconciledTransactions] =
    await Promise.all([
            ...
            >
              ...
              <div className='flex items-center space-x-4'>
                ...
                <div className='flex space-x-2'>
                  <form
                    action={`/api/transaction/${transaction.id}/approve`}
                    method='post'
                    className='inline'
                  >
                    <button
                  ...
                  <form
                    action={`/api/transaction/${transaction.id}/reject`}
                    method='post'
                    className='inline'
                  >
                    <button
