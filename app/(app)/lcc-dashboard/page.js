...
export default async function LCCDashboard() {
  ...
  const [
    ...
    prisma.requisition.count({
      where: { status: 'PENDING', subDistrictId: session.user.subDistrictId }
    }),
    prisma.project.count({
      where: { subDistrictId: session.user.subDistrictId, status: 'ACTIVE' }
    }),
            ...
            {congregationActivities.map(congregation => (
              ...
              >
                <div>
                  ...
                  <p className='text-xs text-gray-500'>
                    {congregation.memberCount} members
                  </p>
                </div>
                <div className='text-right'>
                  <div className='text-xs text-gray-500'>
                    {congregation._count.projects} projects •{' '}
                    {congregation._count.requisitions} requisitions
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
          ...
          >
            ...
            <div className='text-sm text-green-700'>
              Oversee local project portfolio
            </div>
          </a>
          <a
