...
export default async function AgencyLeaderDashboard() {
  ...
  const [
    ...
    prisma.requisition.count({
      where: { status: 'PENDING', agencyId: session.user.agencyId }
    }),
    prisma.budget.findFirst({
      where: { agencyId: session.user.agencyId, year: new Date().getFullYear() }
    }),
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='mt-2 text-gray-600'>
              Manage your agency operations and team
            </p>
          </div>
