...
export default async function FundTransfer({ searchParams }) {
  ...
  const [totalTransfers, pendingTransfers, approvedTransfers, completedTransfers] = await Promise.all([
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Fund Transfer Management</h1>
        <p className="mt-2 text-gray-600">Manage internal and external fund transfers between accounts</p>
      </div>
        ...
        <div className="metric-card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{approvedTransfers}</p>
            </div>
          </div>
        </div>
      ...
      <div className="card">
        ...
        {items.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">No transfer requests found</h3>
            <p className="mt-1 text-sm text-gray-500">
              {status ? 'Try adjusting your filter criteria.' : 'Get started by creating a new transfer request.'}
            </p>
          </div>
        ) : (
                  ...
                  <tr key={transfer.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`status-badge ${
                        transfer.status === 'PENDING' ? 'status-pending' :
                        transfer.status === 'APPROVED' ? 'status-approved' :
                        transfer.status === 'COMPLETED' ? 'status-completed' :
                        'status-rejected'
                      }`}>
                        {transfer.status}
                      </span>
