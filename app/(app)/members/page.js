  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Manage church members and their information</p>
      </div>
        ...
        {items.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {q ? 'Try adjusting your search terms.' : 'Get started by adding a new member.'}
            </p>
          </div>
        ) : (
                  ...
                  <tr key={member.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`status-badge ${
                        member.status === 'ACTIVE' ? 'status-approved' :
                        member.status === 'INACTIVE' ? 'status-rejected' :
                        'status-pending'
                      }`}>
                        {member.status}
                      </span>
