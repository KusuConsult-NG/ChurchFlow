  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Monitor and manage compliance issues across the district</p>
      </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {complianceIssues.filter(issue => issue.status === 'OPEN').length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="metric-card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {complianceIssues.filter(issue => issue.status === 'IN_PROGRESS').length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="metric-card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {complianceIssues.filter(issue => issue.status === 'RESOLVED').length}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="card mb-8">
        <div className="card-header">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Compliance Issues</h2>
              <p className="text-gray-600">Track and manage compliance issues</p>
            </div>
            <button
                ...
                <tr key={issue.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`status-badge ${
                      severityLevels.find(s => s.id === issue.severity)?.color || 'bg-gray-100 text-gray-800'
                    }`}>
                      {severityLevels.find(s => s.id === issue.severity)?.name || issue.severity}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`status-badge ${
                      statusOptions.find(s => s.id === issue.status)?.color || 'bg-gray-100 text-gray-800'
                    }`}>
                      {statusOptions.find(s => s.id === issue.status)?.name || issue.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
        ...
        {complianceIssues.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">Great! No compliance issues to track at the moment.</p>
          </div>
        )}
      </div>
      ...
      {showCreateForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Create Compliance Issue</h2>
