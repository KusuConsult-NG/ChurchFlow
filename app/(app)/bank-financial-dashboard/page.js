...
export default async function BankFinancialDashboard() {
  ...
  const [totalDeposits, totalWithdrawals, pendingTransactions, accountBalances, recentTransactions] = await Promise.all([
    prisma.transaction.aggregate({
      where: { type: 'DEPOSIT', createdAt: { gte: new Date(new Date().getFullYear(), new Date().getMonth(), 1) } },
      _sum: { amount: true }
    }),
    prisma.transaction.aggregate({
      where: { type: 'WITHDRAWAL', createdAt: { gte: new Date(new Date().getFullYear(), new Date().getMonth(), 1) } },
      _sum: { amount: true }
    }),
  ...
  return (
    <main className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">Monthly Deposits</p>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">Monthly Withdrawals</p>
              <p className="text-2xl font-semibold text-gray-900">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center">
            <div className={`p-2 rounded-lg ${netFlow >= 0 ? 'bg-blue-100' : 'bg-orange-100'}`}>
              <svg className={`w-6 h-6 ${netFlow >= 0 ? 'text-blue-600' : 'text-orange-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
              </svg>
            </div>
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">Net Cash Flow</p>
              <p className={`text-2xl font-semibold ${netFlow >= 0 ? 'text-gray-900' : 'text-orange-600'}`}>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="space-y-3">
            {accountBalances.map((account) => (
              <div key={account.name} className="flex items-center justify-between py-2 border-b border-gray-100">
                <div>
      ...
      {/* Quick Actions */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Bank Operations</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <a 
            href="/admin/transactions" 
            className="p-4 text-center bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors"
          >
            <div className="font-medium text-blue-900">Process Transactions</div>
            <div className="text-sm text-blue-700">Review and approve pending transactions</div>
          </a>
          <a 
            href="/admin/bank-reconciliation" 
            className="p-4 text-center bg-green-50 hover:bg-green-100 rounded-lg transition-colors"
          >
            <div className="font-medium text-green-900">Bank Reconciliation</div>
            <div className="text-sm text-green-700">Reconcile accounts with bank statements</div>
          </a>
          <a 
          ...
          >
            ...
            <div className="text-sm text-purple-700">Generate comprehensive financial reports</div>
          </a>
        </div>
      </div>
    </main>
