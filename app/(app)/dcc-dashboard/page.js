...
export default async function DCCDashboard() {
  ...
  const [pendingRequisitions, districtProjects, totalBudget, monthlyExpenses, districtAnnouncements, complianceIssues] = await Promise.all([
    prisma.requisition.count({ where: { status: 'PENDING', districtId: session.user.districtId } }),
    prisma.project.count({ where: { districtId: session.user.districtId, status: 'ACTIVE' } }),
    prisma.budget.aggregate({ 
      where: { districtId: session.user.districtId, year: new Date().getFullYear() },
      _sum: { amount: true }
    }),
    ...
    prisma.announcement.count({ where: { districtId: session.user.districtId } }),
    prisma.complianceIssue.count({ where: { districtId: session.user.districtId, status: 'OPEN' } })
  ]);
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{districtProjects}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{complianceIssues}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          >
            ...
            <div className="text-sm text-blue-700">Review and approve district requisitions</div>
          </a>
          <a 
          ...
          >
            ...
            <div className="text-sm text-green-700">Oversee district project portfolio</div>
          </a>
          <a 
            href="/admin/district-announcements" 
            className="p-4 text-center bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors"
          >
            <div className="font-medium text-purple-900">District Announcements</div>
            <div className="text-sm text-purple-700">Communicate with district agencies</div>
          </a>
          <a 
            href="/admin/compliance-monitoring" 
            className="p-4 text-center bg-red-50 hover:bg-red-100 rounded-lg transition-colors"
          >
            <div className="font-medium text-red-900">Compliance Monitoring</div>
            <div className="text-sm text-red-700">Monitor regulatory compliance</div>
          </a>
        </div>
      </div>
    </main>
