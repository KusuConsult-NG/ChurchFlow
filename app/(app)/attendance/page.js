...
export default async function Attendance({ searchParams }) {
  ...
  const [events, items, total] = await Promise.all([
  ...
  const [totalAttendance, presentCount, absentCount, lateCount] = await Promise.all([
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Track member attendance for events and services</p>
      </div>
        ...
        {items.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {eventId ? 'Try selecting a different event.' : 'Get started by marking attendance for an event.'}
            </p>
          </div>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {items.map((attendance) => (
                  <tr key={attendance.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{attendance.event.title}</div>
                      <div className="text-sm text-gray-500">{attendance.event.location}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`status-badge ${
                        attendance.status === 'PRESENT' ? 'status-approved' :
                        attendance.status === 'ABSENT' ? 'status-rejected' :
                        'status-pending'
                      }`}>
                        {attendance.status}
                      </span>
